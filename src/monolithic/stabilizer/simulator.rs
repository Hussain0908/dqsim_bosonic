...
impl StabilizerState {
    ...
    pub fn apply_h(&mut self, qubit: usize) {
        ...
        for r in 0..(2 * n) {
            let tmp = self.tableau[r][qubit];
            self.tableau[r][qubit] = self.tableau[r][qubit + n];
            self.tableau[r][qubit + n] = tmp;
...
mod tests {
    ...
    fn test_stabilizer_initialization() {
        let sim = StabilizerState::new(2);

        assert!(sim.tableau[2][2]);
        assert!(sim.tableau[3][3]);

        assert!(!sim.tableau[2][4]);
        assert!(!sim.tableau[3][4]);
    }
    ...
    fn test_hadamard_gate() {
        ...
        assert!(sim.tableau[1][0], "Stabilizer should be X after H");
        assert!(!sim.tableau[1][1], "Z bit should be cleared after H");
    }
    ...
    fn test_bell_state_entanglement() {
        ...
        assert!(sim.tableau[2][0], "Should have X on qubit 0");
        assert!(sim.tableau[2][1], "Should have X on qubit 1");

        assert!(sim.tableau[3][2], "Should have Z on qubit 0");
        assert!(sim.tableau[3][3], "Should have Z on qubit 1");

        let mut rng = ChaCha8Rng::seed_from_u64(42);

        let outcome_0 = sim.measure_qubit(0, &mut rng);
        let outcome_1 = sim.measure_qubit(1, &mut rng);

        assert_eq!(
            outcome_0, outcome_1, " Entangled Bell state measurements did not match! Corrupted tableau tracking."
        )
    }
}